      ...
      <section className="w-full px-6 py-32 lg:py-48 flex flex-col items-center border-t border-white/[0.03]">
        <div className="w-full max-w-5xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {SERVICES.map((s, i) => (
              <motion.div key={s.id} {...fadeUp(i * 0.1)} className="p-8 lg:p-10 border border-white/5 rounded-2xl bg-[#080808] flex flex-col">
                <h3 className="font-display text-xl text-lael-light font-bold mb-4">{s.title}</h3>
                <p className="text-lael-muted/60 text-sm leading-relaxed mb-8 flex-1">{s.desc}</p>
      ...
      <section className="w-full px-6 py-32 lg:py-48 flex flex-col items-center border-t border-white/[0.03]">
        <motion.div {...fadeUp(0)} className="text-center max-w-2xl">
          ...
          <button onClick={handleContact}
            className="bg-lael-accent text-lael-primary px-12 py-6 rounded-xl text-xs tracking-[0.2em] uppercase font-bold hover:scale-[1.02] active:scale-95 transition-all duration-500 shadow-[0_0_30px_rgba(198,166,107,0.25)] hover:shadow-[0_0_60px_rgba(198,166,107,0.4)]">
            Solicitar propuesta →
          </button>
