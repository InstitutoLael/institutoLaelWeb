    ...
    <div className="w-full bg-[#F4F4F4] text-[#071D49] overflow-x-clip font-sans">
      ...
      <section className="relative -mt-20 pt-40 pb-28 px-6 text-white overflow-hidden" style={{ backgroundColor: BLUE }}>
        ...
        <div className="relative z-10 max-w-5xl mx-auto text-center">
          ...
          <motion.div {...fadeUp(0.3)} className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 border border-white/25 hover:border-white text-white font-display font-extrabold text-xs uppercase tracking-widest px-8 py-5 rounded-2xl transition-all active:scale-95">
      ...
      <section className="px-6 py-28">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <motion.p {...fadeUp(0)} className="text-[10px] font-bold uppercase tracking-[0.4em] mb-4">Qué hacemos</motion.p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {EMPRESAS_SERVICES.map((s, i) => (
              <motion.div
                key={s.id}
                {...fadeUp(i * 0.08)}
                className={`rounded-[32px] p-8 border flex flex-col ${s.soon ? 'bg-transparent border-dashed border-[#071D49]/20 opacity-70' : 'bg-white border-[#071D49]/5 shadow-card'}`}
              >
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-6" style={{ backgroundColor: BLUE }}>
                  <s.icon size={24} style={{ color: YELLOW }} />
                </div>
                <h3 className="font-display text-lg font-extrabold uppercase tracking-tight mb-3">{s.title}</h3>
                <p className="text-[#8D8D8D] text-sm leading-relaxed mb-6 flex-grow">{s.desc}</p>
      ...
      <section className="px-6 py-24 text-white" style={{ backgroundColor: BLUE }}>
        <div className="max-w-5xl mx-auto">
          <motion.h2 {...fadeUp(0)} className="font-display text-3xl sm:text-4xl font-extrabold uppercase tracking-tight text-center mb-14">
            Empresas que ya trabajaron con nosotros
          </motion.h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {COMPANY_TESTIMONIALS.map((c, i) => (
              <motion.div key={c.id} {...fadeUp(i * 0.1)} className="rounded-[32px] p-8 bg-white/5 border border-white/10">
      ...
      {/* ── COTIZAR ──────────────────────────────────────────────────── */}
      <section id="cotizar" className="px-6 py-28 flex flex-col items-center scroll-mt-24">
        <EmpresasForm />
      </section>
    </div>
  );
