...
// Importa tu logo local aquí. Si falla, el código tiene un fallback visual.
import losOlivosLogo from "../assets/img/Partners/LosOlivos.png"; 

/* ──────────────────────────────────────────────────────────────────────────
   1. ICONOS SVG (Componentes funcionales ligeros)
   ────────────────────────────────────────────────────────────────────────── */
const Icons = {
...
export default function Homeschool() {
  ...
  // 2. Calcular Totales (Lógica especial para Pack Excelencia: Matrícula Gratis)
  // Nota: Si el pack es p12, asumimos matrícula gratis por la descripción de features.
  // Podemos hacerlo dinámico chequeando si incluye la string "Matrícula BONIFICADA" o por ID.
...
--------------------------------
💰 *Total:* ${clp(total)}
${isEnrollmentFree ? '✅ Matrícula Bonificada (Gratis)' : `(Incluye matrícula: ${clp(ENROLLMENT_FEE)})`}

¿Me ayudan a coordinar el horario?`;
    return `https://wa.me/56964626568?text=${encodeURIComponent(text)}`;
  }, [activeLevel, activeSubject, activePack, total, isEnrollmentFree]);
  ...
  // 4. Scroll suave al configurador al hacer click en "Empezar" (opcional)
          ...
          <div className="hero-content">
            ...
            <div className="toggle-pill">
                <button 
                  ...
                  aria-label="Ver opciones para estudiantes"
                >
                ...
                <button 
                  ...
                  aria-label="Ver opciones para colegios"
                >
        ...
        {!isSchool && (
            <div className="animate-fade">
                ...
                <section className="step-section">
                    ...
                    {/* Descripción dinámica de la materia seleccionada */}
                        ...
                        <div className="packs-col">
                            <div className="step-header">
                                ...
                                <div>
                                  ...
                                  <p className="step-sub">Planes mensuales flexibles sin contratos forzosos.</p>
                                </div>
                            </div>
                            ...
                            <div className="summary-card" style={{borderColor: activeSubject.color}}>
                                ...
                                <div className="sum-total">
                                    ...
                                    <div className="total-stack">
                                      ...
                                      <span className="total-note">Pago Mensual</span>
                                    </div>
                                </div>

                                <a href={waLinkStudent} target="_blank" rel="noreferrer" className="btn-checkout">
                                    Agendar Horas <Icons.ArrowRight />
                                </a>
                                
                                <div className="guarantee-box">
                                  <p>✅ <strong>Garantía de Satisfacción:</strong> Si la primera clase no te convence, te cambiamos de tutor o te devolvemos el dinero.</p>
                                </div>
                            </div>
                        </div>
                ...
                <div className="mobile-bar">
                    <div className="mb-info">
                        <span>Total (Matrícula {isEnrollmentFree ? 'GRATIS' : 'inc.'})</span>
...
h1 { 
  ...
  line-height: 1.05; font-weight: 800; letter-spacing: -0.03em; margin-bottom: 20px; 
}
.highlight { 
...
.t-btn {
  ...
  background: transparent; color: var(--text-muted); font-weight: 600; font-size: 0.95rem; transition: 0.3s;
}
...
.partner-float {
  ...
  padding: 16px 20px; border-radius: 16px; border: 1px solid var(--border);
  box-shadow: 0 20px 40px rgba(0,0,0,0.5); display: flex; align-items: center; gap: 15px;
  animation: float 6s ease-in-out infinite; max-width: 300px;
}
...
.step-num { 
  width: 32px; height: 32px; background: #222; border: 1px solid var(--border); border-radius: 50%; 
  display: flex; align-items: center; justify-content: center; font-size: 0.9rem; font-weight: 700; color: white; flex-shrink: 0;
}
...
.subject-card {
  ...
  cursor: pointer; transition: all 0.2s; position: relative; overflow: hidden; display: flex; flex-direction: column; align-items: center; text-align: center;
}
...
.subject-detail-bar {
  margin-top: 20px; background: rgba(255,255,255,0.03); padding: 12px 20px; border-radius: 12px;
  font-size: 0.9rem; color: #ccc; border-left: 3px solid transparent; transition: border 0.3s;
}

/* STEP 2: LEVELS */
.grid-levels { display: flex; gap: 12px; flex-wrap: wrap; }
.level-pill {
  background: var(--bg-card); border: 1px solid var(--border); padding: 10px 20px; border-radius: 12px;
  color: var(--text-muted); text-align: left; transition: 0.2s; min-width: 140px;
}
...
.badge-pop { 
  position: absolute; top: 12px; right: 12px; background: var(--accent-primary); color: white;
  font-size: 0.7rem; font-weight: 800; padding: 4px 10px; border-radius: 6px; text-transform: uppercase;
}
...
.summary-card {
  ...
  padding: 30px; position: sticky; top: 30px; transition: border-color 0.3s;
}
...
.btn-checkout {
  background: white; color: black; width: 100%; padding: 16px; border-radius: 16px; display: flex; align-items: center; justify-content: center; gap: 10px;
  font-weight: 800; font-size: 1.1rem; margin-top: 24px; transition: 0.2s; text-align: center;
}
...
.serv-item { 
  background: var(--bg-card); padding: 30px; border-radius: 20px; border: 1px solid var(--border); transition: 0.2s; 
  display: flex; flex-direction: column; height: 100%;
}
.serv-item:hover { border-color: #3b82f6; transform: translateY(-5px); }
.serv-icon-circle { 
  width: 50px; height: 50px; background: rgba(59, 130, 246, 0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center;
...
.btn-b2b {
  background: white; color: black; padding: 16px 32px; border-radius: 12px; font-weight: 800; font-size: 1.1rem;
...
.mobile-bar {
  ...
  background: rgba(10,10,10,0.9); backdrop-filter: blur(20px); border-top: 1px solid var(--border);
...
@media (max-width: 900px) {
  ...
  .visual-bento { display: none; } /* En móvil priorizamos contenido, ocultamos imagen pesada */
  .partner-float { display: none; } /* Ocultamos float en móvil */
  
  .config-container { grid-template-columns: 1fr; }
  .summary-col { display: none; } /* Se usa mobile bar */
