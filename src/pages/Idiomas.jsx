...
/* ──────────────────────────────────────────────────────────────────────────
   1. DATOS ESTÁTICOS DE UI (Lo que es puro texto/marketing)
   ────────────────────────────────────────────────────────────────────────── */
const STATS_DATA = [
...
/* ──────────────────────────────────────────────────────────────────────────
   2. ESTILOS CSS "LUMINOUS SLATE" (El diseño visual potente)
   ────────────────────────────────────────────────────────────────────────── */
const css = `
:root {
  --bg-deep: #020617;
...
.idiomas-page {
  ...
  position: relative;
  overflow-x: hidden;
  padding-bottom: 140px; /* Espacio para la sticky bar */
}
...
.badge-pill { 
    ...
    font-size: 0.85rem; font-weight: 700; margin-bottom: 24px; text-transform: uppercase;
}
...
.btn-lg { 
    padding: 16px 36px; border-radius: 14px; font-weight: 700; font-size: 1.1rem; 
    transition: all 0.3s;
}
.btn-primary { 
...
.image-card img { 
    width: 100%; border-radius: var(--radius-lg); border: 1px solid var(--glass-border); 
    box-shadow: 0 25px 50px -12px rgba(0,0,0,0.5); filter: brightness(0.9) contrast(1.1);
}
...
.step .circle { 
    position: absolute; left: -41px; width: 32px; height: 32px; background: var(--bg-deep); 
...
.course-card.is-selected { 
    border-color: var(--primary); background: linear-gradient(180deg, rgba(99,102,241,0.08), var(--bg-card)); 
...
.check-indicator { 
    width: 28px; height: 28px; border: 2px solid var(--glass-border); border-radius: 50%; 
...
.lv-btn { 
    flex: 1; padding: 8px 0; font-size: 0.85rem; font-weight: 700; color: var(--text-muted); 
    border-radius: 8px; transition: .2s; text-align: center;
}
...
.btn-select { 
    width: 100%; padding: 14px; border-radius: 12px; font-weight: 700; margin-top: auto;
    background: var(--bg-panel); color: white; border: 1px solid var(--glass-border); 
}
...
.feat-card { 
    background: var(--bg-panel); padding: 30px; border-radius: var(--radius-lg); border: 1px solid var(--glass-border); text-align: center;
}
.feat-icon { font-size: 2.5rem; margin-bottom: 15px; display: block; }

.faq-wrapper details { 
    border: 1px solid var(--glass-border); border-radius: 12px; margin-bottom: 12px; background: var(--bg-card); 
}
...
export default function Idiomas() {
  ...
  // LOGICA DEL CARRITO (Conectado a tu archivo idioma.js)
  ...
  const setLevel = (langId, level) => {
    if (!selectedIds.includes(langId)) toggleCourse(langId, false);
    setSelectedLevels(prev => ({ ...prev, [langId]: level }));
  };

  const waMsg = `Hola, quiero inscribirme en: ${selectedCourses.map(c => c.name + (selectedLevels[c.id] ? ` (${selectedLevels[c.id]})` : '')).join(' + ')}`;

  // EFECTO DE NÚMEROS (CONTADOR)
  useEffect(() => {
      ...
      <section className="highlight-section">
        <div className="container highlight-grid">
          <div>
            ...
            <a href="#" className="btn-ghost" style={{padding:'10px 20px', borderRadius:'8px', display:'inline-block'}}>Pedir test de nivel →</a>
          </div>
        ...
        <div className="container">
          ...
          <div className="courses-grid">
            {LANGUAGES.map((l) => {
              ...
              const currentLvl = selectedLevels[l.id];
                    ...
                    <div className="level-selector">
                      ...
                      <div className="ls-buttons">
                        {displayLevels.slice(0,4).map(lv => (
                          <button key={lv} 
                            className={`lv-btn ${currentLvl === lv ? 'active' : ''}`}
        ...
        <div className="sticky-bar">
            ...
            <div style={{display:'flex', alignItems:'center', gap:'30px'}}>
                <div style={{textAlign:'right'}}>
                    <small style={{display:'block', color:'var(--text-muted)', textTransform:'uppercase', fontSize:'0.75rem'}}>Total Mensual</small>
                    <span style={{color:'white', fontWeight:800, fontSize:'1.8rem'}}>{clp(pricing.totalMonthly)}</span>
                </div>
                <a 
                  href={`https://wa.me/56964626568?text=${encodeURIComponent(waMsg)}`} 
