...
/* ──────────────────────────────────────────────────────────────────────────
   1. UTILS & ICONS
   ────────────────────────────────────────────────────────────────────────── */
// Formateador de moneda seguro (por si el del contexto falla)
const clp = (amount) => {
...
export default function Inscripciones() {
  // Intentamos obtener del contexto, con valores por defecto para no romper
  const context = useCart();
  const cartItems = context?.cartItems || context?.cart || []; // Soporta ambos nombres
  ...
  // Pre-llenado desde carrito
  useEffect(() => {
    if (cartItems.length > 0) {
      const resumen = cartItems.map(i => i.title || i.nombre).join(" + ");
      setForm(prev => ({
        ...prev,
        program: resumen,
        comments: prev.comments || `Compra Web. Total: ${clp(total)}`
      }));
    }
  }, [cartItems, total]);
  ...
  const handleSubmit = async (e) => {
    ...
    const finalProgram = cartItems.length > 0 
        ? cartItems.map(item => `${item.title} (${clp(item.price)})`).join(" + ")
        : form.program;
