    ...
    <section className="paes" style={{ "--accent": "#5850EC", "--accentSoft": "rgba(88,80,236,.10)" }}>
      ...
      <section className="benefits" aria-label="Beneficios clave">
        <div className="container benefits__row" role="list">
          ...
          <div className="benefit" role="listitem">🤝 Tutorías y seguimiento</div>
          <div className="benefit" role="listitem">💳 Matrícula única {clp(ENROLLMENT_FEE)}</div>
        </div>
      </section>
        ...
        <section className="value">
          ...
          <div className="value-grid" role="list">
            <CardPrice
              tag="1 ramo"
              money={clp(annual1)}
              sub={`≈ ${clp(priceForCount(1))} / mes`}
              points={["Ensayo mensual", "Material descargable"]}
              color="indigo"
            />
            <CardPrice
              tag="2 ramos"
              money={clp(annual2)}
              sub={`≈ ${clp(priceForCount(2))} / mes`}
              points={["2 ensayos/mes", "Soporte por WhatsApp"]}
              color="green"
            />
            <CardPrice
              tag="3 ramos"
              money={clp(annual3)}
              sub={`≈ ${clp(priceForCount(3))} / mes`}
              points={["Tutoría mensual", "Cápsulas on-demand"]}
              color="amber"
            />
            <CardPrice
              tag="Todos los ramos"
              ribbon="FULL"
              money={clp(annual7)}
              sub={`≈ ${clp(priceForCount(7))} / mes`}
              points={["7 ensayos/mes", "Soporte premium"]}
              color="rose"
              featured
            />
          </div>

          <div className="note muted tiny">
            Mostramos precio anual para comparar fácil. Matrícula única {clp(ENROLLMENT_FEE)}.
          </div>
        </section>
          ...
          <div className="summary" aria-describedby="precios-live">
            <div className="sum-left">
              ...
              <ul className="sum-feats">
                ...
                <li>
                  {subjectCount ? `${essaysForCount(subjectCount)} ensayo(s)/mes` : "Ensayos mensuales"}
                </li>
            ...
            <div className="sum-price">
              ...
              <div id="precios-live" className="sr-only" aria-live="polite">
                Precio mensual {subjectCount ? clp(monthly) : "no disponible"}. Precio anual{" "}
                {subjectCount ? clp(annual) : "no disponible"}.
              </div>
            </div>
          </div>
        </section>
...
.benefits{ padding:12px 0 8px; border-bottom:1px solid var(--bd); }
.benefits__row{ display:grid; gap:10px; grid-template-columns:repeat(4,1fr); }
@media (max-width:980px){ .benefits__row{ grid-template-columns:repeat(2,1fr); } }
@media (max-width:560px){ .benefits__row{ grid-template-columns:1fr; } }
.benefit{
...
.value-card{
  ...
  padding:14px; color:#fff; box-shadow:0 10px 20px rgba(2,6,23,.22);
  overflow:hidden;
}
...
.combo-card:hover{ transform: translateY(-2px); box-shadow:0 22px 44px rgba(2,6,23,.28); border-color:#2a3550; }
.badge{
...
.subject{
  padding:.52rem .78rem; border-radius:999px; border:1px solid #28324a;
  background:#0f172a; color:#e5e7eb; font-weight:900; transition:.15s ease;
}
...
/* Sticky CTA móvil */
.paes{ padding-bottom:70px; } /* evita que la sticky tape el contenido */
.sticky-cta{
