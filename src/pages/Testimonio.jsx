...
export default function Testimonio() {
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    if (f.nombre.trim().length < 2 || f.testimonio.trim().length < 10) { setError('Cuéntanos tu nombre y tu experiencia (al menos una frase).'); return; }
    if (!f.acepta_privacidad) { setError('Debes aceptar la política de privacidad.'); return; }
    setError('');
    setEstado('enviando');
    if (backendReady()) {
      try {
        ...
        {estado === 'ok' ? (
          ...
          <form onSubmit={onSubmit} noValidate className="max-w-2xl mx-auto bg-white rounded-[28px] p-5 sm:p-8 border border-[#071D49]/5 shadow-card space-y-4">
            <div aria-hidden="true" className="absolute -left-[9999px] w-px h-px overflow-hidden">
              <input tabIndex={-1} autoComplete="off" value={f.sitio_web} onChange={set('sitio_web')} />
            </div>
            <div><label htmlFor="t-nombre" className={LABEL}>Tu nombre</label><input id="t-nombre" className={INPUT} value={f.nombre} onChange={set('nombre')} autoComplete="name" /></div>
            ...
            <div><label htmlFor="t-texto" className={LABEL}>¿Qué cambió en ti estudiar en Lael?</label><textarea id="t-texto" rows={4} className={`${INPUT} py-3`} value={f.testimonio} onChange={set('testimonio')} /></div>
            <div><label htmlFor="t-rec" className={LABEL}>¿Qué es lo que más recuerdas? <span className="font-normal text-[#071D49]/50">(opcional)</span></label><textarea id="t-rec" rows={3} className={`${INPUT} py-3`} value={f.recuerdo} onChange={set('recuerdo')} /></div>
            <fieldset>
            ...
            <div><label htmlFor="t-correo" className={LABEL}>Tu correo <span className="font-normal text-[#071D49]/50">(opcional, no se publica)</span></label><input id="t-correo" type="email" className={INPUT} value={f.correo} onChange={set('correo')} /></div>
