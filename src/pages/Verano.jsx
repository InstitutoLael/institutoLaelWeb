  ...
  return (
    <div className="w-full bg-[#F4F4F4] text-[#071D49] overflow-x-clip font-sans">
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          {VERANO_COURSES.map((c, i) => (
            <motion.div
              key={c.id}
              {...fadeUp(i * 0.08)}
              className={`relative rounded-[28px] p-6 sm:p-8 flex flex-col border ${c.featured ? 'bg-[#071D49] text-white border-[#071D49] shadow-lael' : 'bg-white border-[#071D49]/5 shadow-card'}`}
            >
              ...
              <p className={`text-sm leading-relaxed mb-6 ${c.featured ? 'text-white/75' : 'text-[#071D49]/70'}`}>{c.desc}</p>
              ...
              <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                <div>
                  ...
                  <p className={`text-xs ${c.featured ? 'text-white/60' : 'text-[#071D49]/70'}`}>{c.priceNote}</p>
                </div>
                <a
                  href={VERANO_WHATSAPP(c.name)}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`inline-flex items-center justify-center gap-2 w-full sm:w-auto min-h-[48px] font-display font-extrabold text-xs sm:text-sm uppercase tracking-wider px-6 py-4 rounded-2xl transition-all active:scale-95 ${c.featured ? 'bg-[#D7E400] text-[#071D49] hover:bg-white' : 'bg-[#071D49] text-white hover:bg-[#0B2A66]'}`}
                >
      ...
      <section className="px-5 sm:px-6 pb-16 sm:pb-20 lg:pb-28">
        <motion.div {...fadeUp(0)} className="max-w-3xl mx-auto text-center rounded-[28px] bg-white p-6 sm:p-10 border border-[#071D49]/5 shadow-card">
          ...
          <p className="text-[#071D49]/70 leading-relaxed mb-6 sm:mb-8">
            La primera semana de marzo empiezan el preu PAES, inglés y la Escuela de Sueños. La matrícula es gratis.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link to="/paes" className="inline-flex items-center justify-center gap-2 bg-[#071D49] text-white hover:bg-[#0B2A66] min-h-[48px] font-display font-extrabold text-xs sm:text-sm uppercase tracking-wider px-6 py-4 rounded-2xl">Preu PAES</Link>
            <Link to="/idiomas" className="inline-flex items-center justify-center gap-2 border-2 border-[#071D49]/20 hover:border-[#071D49] min-h-[48px] font-display font-extrabold text-xs sm:text-sm uppercase tracking-wider px-6 py-4 rounded-2xl">Inglés</Link>
            <Link to="/adultos" className="inline-flex items-center justify-center gap-2 border-2 border-[#071D49]/20 hover:border-[#071D49] min-h-[48px] font-display font-extrabold text-xs sm:text-sm uppercase tracking-wider px-6 py-4 rounded-2xl">Escuela de Sueños</Link>
          </div>
        </motion.div>
      </section>
    </div>
